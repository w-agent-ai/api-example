#!/usr/bin/env python3
"""
Minimal buyer-side x402 demo for the public face feature API.

Flow:
1. Read a local aligned face image
2. POST /v1/public/features/face
3. The x402 client handles HTTP 402, signs payment, and retries
4. Print the face feature result JSON
"""
...
# The face API expects one already aligned/corrected face image.
# If the original image contains a full scene, detect and align the face locally
# first, then point this path to the aligned face crop.
...
def main() -> int:
    ...
    if not private_key:
        print("edit EVM_PRIVATE_KEY in anonymous_face_x402_demo.py before running this demo", file=sys.stderr)
        return 2
    ...
    url = f"{BASE_URL.rstrip('/')}/v1/public/features/face"

    print(f"base_url={BASE_URL}")
    print(f"image_path={IMAGE_PATH}")
    print("starting paid face feature...")
