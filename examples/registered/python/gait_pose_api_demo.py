#!/usr/bin/env python3
"""
Registered-user demo for human 2D/3D keypoint extraction from an existing image sequence.
...
# Optional local output directory if this file is imported by video demos.
RESULT_DIR = Path("./result")

# Network timeout in seconds. Keypoint extraction may take longer for long sequences.
TIMEOUT = 120
...
def main() -> int:
    ...
    with requests.Session() as session:
        result = run_registered_gait_pose(session, {"Authorization": f"Bearer {api_key}"}, SEQ_DIR)
...
def load_api_key() -> str:
    ...
    if not api_key or api_key == "gak_your_api_key":
        print("edit API_KEY in gait_pose_api_demo.py before running this demo", file=sys.stderr)
        raise SystemExit(2)
    return api_key


def run_registered_gait_pose(session: requests.Session, headers: dict[str, str], seq_dir: Path) -> dict:
    frames = collect_frames(seq_dir)
    ...
    # /gait-pose runs standalone human 2D/3D keypoint extraction and bills the
    # registered account separately from gait recognition.
    return request_json(
        session,
        "POST",
