...
sequence_root="$(pwd)/${video_stem}_gait_sequences"

./build.sh
"$(pwd)/build/local_video_sequence_extractor" "$video_abs"
...
while IFS= read -r -d '' seq_dir; do
  ...
  GAIT_SEQUENCE_DIR="$seq_dir" "$sequence_demo_dir/build/registered_sequence_demo"
