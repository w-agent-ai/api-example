...
// Send the aligned face crop to the registered-user Face API. Billing happens
// on the server side after the request is accepted.
static std::string postFaceFeature(const std::string& imageBase64) {
  ...
  std::string url = kBaseURL + "/v1/features/face";
...
int main(int argc, char** argv) {
  try {
    ...
    // Detect the best face locally with face_detect.onnx. The server expects an
    // aligned face image, so the original scene image should not be sent directly.
    FaceDetectorONNX detector;
    FaceCandidate face = detectBestFaceONNX(bgr, detector);

    // Align by the detected eye landmarks and JPEG-encode the aligned crop.
    cv::Mat aligned = alignFace(bgr, face);
    std::vector<unsigned char> jpeg;
    cv::imencode(".jpg", aligned, jpeg, {cv::IMWRITE_JPEG_QUALITY, 92});

    // The HTTP API accepts raw base64 without a data:image/... prefix.
    std::string body = postFaceFeature(base64Encode(jpeg.data(), jpeg.size()));
    std::cout << body << "\n";
    return 0;
